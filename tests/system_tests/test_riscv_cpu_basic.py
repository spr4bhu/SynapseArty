...
async def run_test_program(dut, instr_mem):
    ...
    # Feed instructions modeling SYNCHRONOUS instruction memory: the registered
    # read makes the instruction available one cycle AFTER the PC, and it must
    # hold while the core asserts module_fetch_hold (pipeline stall). This matches
    # the real unified_memory after the BRAM / synchronous-fetch change.
    # Match real memory's reset state: the PC holds at 0 through reset, so the
    # registered read already presents instr[0] when execution begins.
    fetched = get_instr(0)
    for cycle in range(60):  # deeper pipeline needs a few more cycles
        # Present the registered instruction (from the previous cycle's PC fetch)
        dut.module_instr_in.value = fetched
        pc = int(dut.module_pc_out.value)
        current_instr = fetched
        # Honor the fetch-hold (pipeline stall); treat unresolved (X) as not held.
        hv = dut.module_fetch_hold.value
        hold = bool(int(hv)) if hv.is_resolvable else False
        next_fetched = fetched if hold else get_instr(pc)
        ...
        # Advance simulation
        await RisingEdge(dut.clk)
        fetched = next_fetched   # synchronous read result becomes visible next cycle
