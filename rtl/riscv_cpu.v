...
module riscv_cpu (
    ...
    // Pipeline stall exported so the instruction memory can freeze its read
    ...
    // bubbles MEM-WB so the load is written back with valid data next cycle.
    reg mem_wait;
    ...
    // Registered fetch: the instruction arrives one cycle after the PC. Delay the
    // PC to match (if_pc_q), and NOP the two wrong-path fetches after a redirect
    // (branch_flush + redirect_q) and the first fetch out of reset (rst_q).
    reg [31:0] if_pc_q;
    reg        redirect_q;
    reg        rst_q;
    always @(posedge clk) begin
        ...
        if (rst) begin
            ...
            if_pc_q    <= pc_inst0_out;
            redirect_q <= branch_flush;
        end
    end

    wire [31:0] instruction_to_if_id;
    assign instruction_to_if_id = (branch_flush || redirect_q || rst_q) ? 32'h00000013
                                                                        : module_instr_in;
    wire if_id_stall;
    ...
    // CSR state exported from csr_file (replaces hierarchical reads of csr_file_inst.*)
