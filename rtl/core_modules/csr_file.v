...
module csr_file (
    ...
    // CSR state exposed as ports for the interrupt controller / execution unit.
    // These were previously read via cross-module hierarchical references
    // (csr_file_inst.mstatus, ...), which Verilator allows but Vivado synthesis
    // rejects. Exposing them as output regs keeps behavior identical.
    ...
    // CSR registers (mstatus, mie, mip, mtvec, mepc are now output-reg ports above)
    reg [31:0] misa;
    reg [31:0] mscratch;
    reg [31:0] mcause;
    reg [31:0] mtval;
