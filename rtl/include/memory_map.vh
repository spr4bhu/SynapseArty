...
// 32KB - on-chip FPGA block RAM (Arty A7-35T). Scaled up now that the memory
// synthesizes to true BRAM (Phase A); well within the xc7a35t's ~225KB of BRAM.
`define INSTR_MEM_BASE      32'h00000000
`define INSTR_MEM_SIZE      32'h00008000  // 32KB
`define INSTR_MEM_END       32'h00007FFF
...
// Data Memory
// 32KB - on-chip FPGA block RAM; scaled up alongside instruction memory (Phase B).
`define DATA_MEM_BASE       32'h10000000
`define DATA_MEM_SIZE       32'h00008000  // 32KB
`define DATA_MEM_END        32'h10007FFF

// Stack grows down from top of data memory
`define STACK_TOP           32'h10007FFF
